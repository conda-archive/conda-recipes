...
class PatchSet(object):
  ...
  def _detect_type(self, p):
    ...
    print('DVCS is %s .. p.source is %s, p.target is %s' % (DVCS, p.source, p.target))
    # GIT type check
  ...
  def apply(self, strip=0, root=None):
    ...
    for i,p in enumerate(self.items):
      ...
      print('filename %s .. old %s, new %s' % (filename, old, new))
      if not filename:
      ...
      if canpatch:
        ...
        if exists(backupname):
          ...
          if self.write_hunks(backupname, filename, p.hunks):
            ...
            print('NEW is %s, filename is %s' % (new, filename))
            if new == b'/dev/null':
