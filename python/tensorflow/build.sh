...
if [ `uname` == Darwin ]; then
    if [ "$PY_VER" == "2.7" ]; then
        pip install https://storage.googleapis.com/tensorflow/mac/tensorflow-0.7.0-py2-none-any.whl
    else
        pip install https://storage.googleapis.com/tensorflow/mac/tensorflow-0.7.0-py3-none-any.whl
    fi
fi
